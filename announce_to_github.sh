...
GITHUB_REPO_URL=https://github.com/JiscRDSS/taxonomyschema
...
build_announcement_text() {
  echo "A new release of the Taxonomy Schema has been created: <$GITHUB_REPO_URL/releases/tag/${TRAVIS_TAG}|$TRAVIS_TAG>." \
...
execute_announcement () {
  curl -X POST \
    ...
    -d "as_user=false" \
    -d "username=Taxonomy Schema Bot" \
    -d "channel=$1" \
    -d "text=$2" \
